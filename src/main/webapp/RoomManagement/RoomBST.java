...
import java.util.List;
    ...
    public class RoomBST {
        ...
        private Node deleteRec(Node root, String roomID) {
            if (root == null) return null;

            if (roomID.compareTo(root.room.getRoomID()) < 0) {
                root.left = deleteRec(root.left, roomID);
            } else if (roomID.compareTo(root.room.getRoomID()) > 0) {
                ...
                root.room = findMin(root.right).room;
